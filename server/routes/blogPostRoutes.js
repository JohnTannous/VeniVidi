...
const getBlogPostByCategory = async (req, res) => {
  ...
  let getStatus = () => (increment < posts.length ? 200 : 201);
  ...
  try {
    ...
    res.status(400).json({ message: 'Something went wrong while loading the blog posts. Please try again later.' });
  }
};
...
const createBlogPost = async (req, res) => {
  const { image, title, contentOne, contentTwo, category, author } = req.body;
